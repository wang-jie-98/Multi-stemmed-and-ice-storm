# 1. readying --------------------------------------------------------------------------------------
# loading packages --
pacman::p_load(openxlsx, tidyverse, ggsci, glmmTMB, performance, emmeans, MuMIn, piecewiseSEM)
...
p1 <- ggeffects::ggpredict(model_01, "bin_stem") |> tibble() |> set_names("type", "estimate", "SE", "lower", "upper", "group") |> 
  ...
  labs(x = "", y = "Probability of tree damage") + Sylvan; p1
...
p2 <- ggeffects::ggpredict(model_01, "bin_stem") |> tibble() |> set_names("type", "estimate", "SE", "lower", "upper", "group") |> 
  ...
  labs(x = "", y = "Probability of tree damage") + Sylvan; p2
...
p3 <- ggeffects::ggpredict(model_01, "bin_stem") |> tibble() |> set_names("type", "estimate", "SE", "lower", "upper", "group") |> 
  ggplot() + geom_line(aes(x = type, y = estimate, group = 1), color = "#005f80") + 
  geom_segment(aes(x = type, y = lower, xend = type, yend = upper), color = "#f48a02") + 
  geom_point(aes(x = type, y = estimate), size = 2, fill = "#ffffff", shape = 21) + 
  geom_text(aes(x = 1.5, y = max(estimate)), label = "***", size = 3.2, family = "serif") + 
  labs(x = "", y = "Probability of tree damage") + Sylvan; p3
...
p4 <- ggeffects::ggpredict(model_01, "bin_stem") |> tibble() |> set_names("type", "estimate", "SE", "lower", "upper", "group") |> 
  ggplot() + geom_line(aes(x = type, y = estimate, group = 1), color = "#005f80") + 
  geom_segment(aes(x = type, y = lower, xend = type, yend = upper), color = "#f48a02") + 
  geom_point(aes(x = type, y = estimate), size = 2, fill = "#ffffff", shape = 21) + 
  geom_text(aes(x = 1.5, y = max(estimate)), label = "***", size = 3.2, family = "serif") + 
  labs(x = "", y = "Probability of tree damage") + Sylvan; p4
...
p5 <- ggeffects::ggpredict(model_01, "bin_stem") |> tibble() |> set_names("type", "estimate", "SE", "lower", "upper", "group") |> 
  ggplot() + geom_line(aes(x = type, y = estimate, group = 1), color = "#005f80") + 
  geom_segment(aes(x = type, y = lower, xend = type, yend = upper), color = "#f48a02") + 
  geom_point(aes(x = type, y = estimate), size = 2, fill = "#ffffff", shape = 21) + 
  geom_text(aes(x = 1.5, y = max(estimate)), label = "***", size = 3.2, family = "serif") + 
  labs(x = "", y = "Probability of tree damage") + Sylvan; p5

models <- str_c("model_", str_pad(1:3, 2, side = "left", pad = "0")); for (i in 1:length(models)) {model <- get(models[i]); model_list <- append(model_list, list(model))}
model_save <- bind_rows(model_save, save_glmm("", 3))

# ______(6) save results ---------------------------------------------------------------------------
cowplot::plot_grid(p1, p2, p3, NULL, p4, p5, nrow = 2) + 
...
# ggsave(file = "save/part1_indi_20260620.tiff", width = 18, height = 12, units = "cm", dpi = 300, limitsize = FALSE, bg = "#ffffff")

# save(model_list, file = "save/part1_indi_list_20260620.rdata")
# save(model_save, file = "save/part1_indi_data_20260620.rdata")
# write.xlsx(model_save, "save/part1_indi_data_20260620.xlsx")

rm(model, models, model_list, model_save, i, model_01, model_02, model_03, p1, p2, p3, p4, p5, data_anal)
...
p1 <- ggplot() + geom_line(data = data_pred, aes(x = avg_stem, y = damage), color = "#ff0000") + 
  ...
  Sylvan + theme(legend.position = "none"); p1
...
p2 <- ggplot() + geom_line(data = data_pred, aes(x = avg_stem, y = damage), color = "#ff0000") + 
  geom_point(data = data_anal, aes(x = avg_stem, y = DR_damage_2), size = 0.1, alpha = 0.5) + 
  geom_text(aes(x = ggpp::as_npc(0.9), y = ggpp::as_npc(0.9)), label = "***", size = 3.2, family = "serif") + 
  Sylvan + theme(legend.position = "none"); p2
...
p3 <- ggplot() + geom_line(data = data_pred, aes(x = avg_stem, y = damage), color = "#ff0000") + 
  geom_point(data = data_anal, aes(x = avg_stem, y = DR_damage_3), size = 0.1, alpha = 0.5) + 
  geom_text(aes(x = ggpp::as_npc(0.9), y = ggpp::as_npc(0.9)), label = "***", size = 3.2, family = "serif") + 
  Sylvan + theme(legend.position = "none"); p3
...
p4 <- ggplot() + geom_line(data = data_pred, aes(x = avg_stem, y = damage), color = "#ff0000") + 
  geom_point(data = data_anal, aes(x = avg_stem, y = DR_damage_4), size = 0.1, alpha = 0.5) + 
  geom_text(aes(x = ggpp::as_npc(0.9), y = ggpp::as_npc(0.9)), label = "*", size = 3.2, family = "serif") + 
  Sylvan + theme(legend.position = "none"); p4
...
p5 <- ggplot() + geom_line(data = data_pred, aes(x = avg_stem, y = damage), color = "#ff0000") + 
  geom_point(data = data_anal, aes(x = avg_stem, y = DR_damage_5), size = 0.1, alpha = 0.5) + 
  geom_text(aes(x = ggpp::as_npc(0.9), y = ggpp::as_npc(0.9)), label = "**", size = 3.2, family = "serif") + 
  Sylvan + theme(legend.position = "none"); p5
...
cowplot::plot_grid(p1, p2, p3, NULL, p4, p5, nrow = 2) + 
  ...
  annotate(geom = "text", x = ggpp::as_npc(0.50), y = ggpp::as_npc(0.48), label = "Up",     size = 3, family = "serif", fontface = "bold") + 
  annotate(geom = "text", x = ggpp::as_npc(0.83), y = ggpp::as_npc(0.48), label = "Branch", size = 3, family = "serif", fontface = "bold")
# ggsave(file = "save/part1_spec_20260620.tiff", width = 18, height = 12, units = "cm", dpi = 300, limitsize = FALSE, bg = "#ffffff")
...
# save(model_list, file = "save/part1_spec_list_20260620.rdata")
# save(model_save, file = "save/part1_spec_data_20260620.rdata")
# write.xlsx(model_save, "save/part1_spec_data_20260620.xlsx")

rm(model, models, model_list, model_save, i, model_01, model_02, model_03, model_04, model_05, p1, p2, p3, p4, p5, data_anal, data_pred)
...
cowplot::plot_grid(p1, p2, p3, NULL, p4, p5, nrow = 2) + 
  ...
  annotate(geom = "text", x = ggpp::as_npc(0.50), y = ggpp::as_npc(0.48), label = "Up",     size = 3, family = "serif", fontface = "bold") + 
  annotate(geom = "text", x = ggpp::as_npc(0.83), y = ggpp::as_npc(0.48), label = "Branch", size = 3, family = "serif", fontface = "bold")
# ggsave(file = "save/part1_comm_20260620.tiff", width = 18, height = 12, units = "cm", dpi = 300, limitsize = FALSE, bg = "#ffffff")

models <- str_c("model_", str_pad(1:5, 2, side = "left", pad = "0")); for (i in 1:length(models)) {model <- get(models[i]); model_list <- append(model_list, list(model))}
model_save <- bind_rows(model_save, save_glmm("", 5))
# save(model_list, file = "save/part1_comm_list_20260620.rdata")
# save(model_save, file = "save/part1_comm_data_20260620.rdata")
# write.xlsx(model_save, "save/part1_comm_data_20260620.xlsx")
...
p1 <- ggplot(data_anal, aes(x = H,               y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p1
p2 <- ggplot(data_anal, aes(x = BA,              y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p2
p3 <- ggplot(data_anal, aes(x = NMB,             y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p3
p4 <- ggplot(data_anal, aes(x = SLA,             y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p4
p5 <- ggplot(data_anal, aes(x = LeafN,           y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p5
p6 <- ggplot(data_anal, aes(x = seed_mass,       y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p6
p7 <- ggplot(data_anal, aes(x = seed_length,     y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p7
p8 <- ggplot(data_anal, aes(x = seed_width,      y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p8
p9 <- ggplot(data_anal, aes(x = dispersal_agent, y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p9
p10 <- ggplot(data_anal, aes(x = height,          y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p10
p11 <- ggplot(data_anal, aes(x = rdmax,           y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p11
p12 <- ggplot(data_anal, aes(x = woody_density,   y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p12
p13 <- ggplot(data_anal, aes(x = P50,             y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p13
p14 <- ggplot(data_anal, aes(x = Trait_PC1,       y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p14
p15 <- ggplot(data_anal, aes(x = Trait_PC2,       y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p15
p16 <- ggplot(data_anal, aes(x = altitude,        y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p16
p17 <- ggplot(data_anal, aes(x = forest_age,      y = n_stem)) + geom_jitter(height = 0, size = 0.1) + Sylvan; p17

cowplot::plot_grid(p1, p2, p3, p14, NULL, p15, p16, NULL, p17, nrow = 3)
# ggsave(file = "save/part2_univ1_20260620.tiff", width = 18, height = 18, units = "cm", dpi = 300, limitsize = FALSE, bg = "#ffffff")

cowplot::plot_grid(p4, p5, NULL, NULL, p6, p7, p8, p9, p10, p11, p12, p13, nrow = 3)
# ggsave(file = "save/part2_univ2_20260620.tiff", width = 24, height = 18, units = "cm", dpi = 300, limitsize = FALSE, bg = "#ffffff")
rm(data_plot, p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14, p15, p16, p17)
...
# save(model_list, file = "save/part2_univ_list_20260620.rdata")
# save(model_save, file = "save/part2_univ_data_20260620.rdata")
# write.xlsx(model_save, "save/part2_univ_data_20260620.xlsx")
...
model_set <- dredge(model_all, trace = 2, options(na.action = "na.fail")); save(model_set, file = "save/model_set_20260620.rdata")
...
# ______(6) save results ---------------------------------------------------------------------------
# save(model_save, file = "save/part3_SEM_data_20260620.rdata")
# write.xlsx(model_save, "save/part3_SEM_data_20260620.xlsx")
...
p1 <- ggeffects::ggpredict(model, "bin_stem") |> tibble() |> set_names("type", "estimate", "SE", "lower", "upper", "group") |> 
  ggplot() + 
  ...
  geom_text(aes(x = ggpp::as_npc(0.76), y = ggpp::as_npc(0.94)), label = expression(italic(P)*" < 0.001***"), size = (9*0.35), family = "serif") + 
  scale_x_discrete(labels = c("single-stemmed    ", "    multi-stemmed")) + 
  scale_y_continuous(limits = c(0.04, 0.18), breaks = c(0.04, 0.11, 0.18), labels = c("4", "11", "18")) + 
  labs(x = "Stem category", y = NULL) + 
  Sylvan; p1
...
p2 <- ggplot() + 
  ...
  scale_y_continuous(limits = c(0, 0.6), breaks = c(0, 0.3, 0.6), labels = c("0", "30", "60")) + 
  labs(x = "Stem abundance", y = NULL) + 
  geom_text(aes(x = ggpp::as_npc(0.76), y = ggpp::as_npc(0.94)), label = expression(italic(P)*" < 0.001***"), size = (9*0.35), family = "serif") + 
  Sylvan + 
...
p3 <- ggplot() + 
  ...
  scale_y_continuous(limits = c(0, 0.8), breaks = c(0, 0.4, 0.8), labels = c("0", "40", "80")) + 
  labs(x = "Stem abundance", y = NULL) + 
  geom_text(aes(x = ggpp::as_npc(0.75), y = ggpp::as_npc(0.94)), label = expression(italic(P)*" < 0.001***"), size = (9*0.35), family = "serif") + 
  Sylvan + 
  theme(legend.position = "none"); p3

# ____3.2.4 Fig 2D ---------------------------------------------------------------------------------
data <- read.xlsx("3_save/statistical models/part1_indi_effect_20251228.xlsx")
p4 <- data |> filter(Parameter != "(Intercept)", model == "model_03", DR != "damage_1") |> 
  ggplot(aes(x = Std_Coefficient, y = desc(DR))) + 
  geom_rect(aes(xmin = ggpp::as_npc(0), xmax = ggpp::as_npc(1), ymin = -0.7, ymax = -2.3), fill = "#d62728", alpha = 0.02) + 
  geom_rect(aes(xmin = ggpp::as_npc(0), xmax = ggpp::as_npc(1), ymin = -4.3, ymax = -2.7), fill = "#2ca02c", alpha = 0.02) + 
  geom_errorbarh(aes(xmax = CI_high, xmin = CI_low, color = DR), height = 0, linewidth = 0.8, alpha = 0.3) + 
  geom_errorbarh(aes(xmax = Std_Coefficient + (CI_high-Std_Coefficient)/1.96, xmin = Std_Coefficient - (Std_Coefficient-CI_low)/1.96, color = DR), height = 0, linewidth = 1.5, alpha = 0.6) + 
  geom_point(aes(color = DR), shape = c(21, 21, 19, 19), fill = "#ffffff", size = 2.5, alpha = 1) + 
  geom_vline(aes(xintercept = 0)) + 
  scale_x_continuous(limits = c(-0.17, 0.13), breaks = c(-0.1, 0, 0.1), labels = c("−0.1", "0", "0.1")) + 
  scale_y_continuous(limits = c(-0.7, -4.3)) + 
  scale_color_manual(values = c("#d62728", "#ff7f0e", "#1f77b4", "#2ca02c")) + 
  labs(x = "", y = "") + 
  Sylvan + 
  theme(legend.position = "none", 
        axis.ticks.y = element_blank(), 
        axis.text.y = element_blank(), 
        plot.margin = margin(t = 2, b = 2, l = 7, r = 2, "pt"), 
        panel.border = element_blank(), 
        axis.line.x = element_line(color = "#000000")); p4

# ____3.2.5 Fig 2E ---------------------------------------------------------------------------------
data <- read.xlsx("3_save/statistical models/part1_spec_effect_20251228.xlsx")
p5 <- data |> filter(Parameter != "(Intercept)", model == "model_01", DR != "damage_1") |> 
  ggplot(aes(x = Std_Coefficient, y = desc(DR))) + 
  geom_rect(aes(xmin = ggpp::as_npc(0), xmax = ggpp::as_npc(1), ymin = -0.7, ymax = -2.3), fill = "#d62728", alpha = 0.02) + 
  geom_rect(aes(xmin = ggpp::as_npc(0), xmax = ggpp::as_npc(1), ymin = -4.3, ymax = -2.7), fill = "#2ca02c", alpha = 0.02) + 
  geom_errorbarh(aes(xmax = CI_high, xmin = CI_low, color = DR), height = 0, linewidth = 0.8, alpha = 0.3) + 
  geom_errorbarh(aes(xmax = Std_Coefficient + (CI_high-Std_Coefficient)/1.96, xmin = Std_Coefficient - (Std_Coefficient-CI_low)/1.96, color = DR), height = 0, linewidth = 1.5, alpha = 0.6) + 
  geom_point(aes(color = DR), shape = c(19, 19, 19, 19), fill = "#ffffff", size = 2.5, alpha = 1) + 
  geom_vline(aes(xintercept = 0)) + 
  scale_x_continuous(limits = c(-1.2, 0.1), breaks = c(-1, -0.5, 0), labels = c("−1", "−0.5", "0")) + 
  scale_y_continuous(limits = c(-0.7, -4.3)) + 
  scale_color_manual(values = c("#d62728", "#ff7f0e", "#1f77b4", "#2ca02c")) + 
  labs(x = "Standardization coefficients", y = "") + 
  Sylvan + 
  theme(legend.position = "none", 
        axis.ticks.y = element_blank(), 
        axis.text.y = element_blank(), 
        plot.margin = margin(t = 2, b = 2, l = 7, r = 2, "pt"), 
        panel.border = element_blank(), 
        axis.line.x = element_line(color = "#000000")); p5

# ____3.2.6 Fig 2F ---------------------------------------------------------------------------------
data <- read.xlsx("3_save/statistical models/part1_comm_effect_20251228.xlsx")
p6 <- data |> filter(Parameter != "(Intercept)", model == "model_01", DR != "damage_1") |> 
  ggplot(aes(x = Std_Coefficient, y = desc(DR))) + 
  geom_rect(aes(xmin = ggpp::as_npc(0), xmax = ggpp::as_npc(1), ymin = -0.7, ymax = -2.3), fill = "#d62728", alpha = 0.02) + 
  geom_rect(aes(xmin = ggpp::as_npc(0), xmax = ggpp::as_npc(1), ymin = -4.3, ymax = -2.7), fill = "#2ca02c", alpha = 0.02) + 
  geom_errorbarh(aes(xmax = CI_high, xmin = CI_low, color = DR), height = 0, linewidth = 0.8, alpha = 0.3) + 
  ...
  geom_point(aes(color = DR), shape = c(21, 21, 19, 19), fill = "#ffffff", size = 2.5, alpha = 1) + 
  geom_vline(aes(xintercept = 0)) + 
  scale_x_continuous(limits = c(-0.21, 0.06), breaks = c(-0.2, -0.1, 0), labels = c("−0.2", "−0.1", "0")) + 
  scale_y_continuous(limits = c(-0.7, -4.3)) + 
  scale_color_manual(values = c("#d62728", "#ff7f0e", "#1f77b4", "#2ca02c")) + 
  labs(x = "", y = "") + 
  Sylvan + 
  theme(legend.position = "none", 
        ...
        plot.margin = margin(t = 2, b = 2, l = 7, r = 2, "pt"), 
        panel.border = element_blank(), 
        axis.line.x = element_line(color = "#000000")); p6
...
p7 <- ggplot() + 
  ...
  annotate(geom = "text", x = 2.5, y = 0.5, label = "Individual level", size = (10*0.35), family = "serif", fontface = "bold"); p7
...
p8 <- ggplot() + 
  ...
  theme(axis.line = element_blank(), 
        ...
        panel.background = element_rect(fill = "#ffffff"), 
        panel.grid = element_blank()) + 
  annotate(geom = "text", x = 2.5, y = 0.5, label = "Species level", size = (10*0.35), family = "serif", fontface = "bold"); p8
...
p9 <- ggplot() + 
  ...
  theme(axis.line = element_blank(), 
        ...
        panel.background = element_rect(fill = "#ffffff"), 
        panel.grid = element_blank()) + 
  annotate(geom = "text", x = 2.5, y = 0.5, label = "Community level", size = (10*0.35), family = "serif", fontface = "bold"); p9

p1 <- cowplot::plot_grid(p7, p8, p9, p1, p2, p3, nrow = 2, rel_heights = c(0.4, 5)) + 
  annotate(geom = "text", x = ggpp::as_npc(0.014), y = ggpp::as_npc(0.97), label = "(a)", size = (10*0.35), family = "serif") + 
  annotate(geom = "text", x = ggpp::as_npc(0.349), y = ggpp::as_npc(0.97), label = "(b)", size = (10*0.35), family = "serif") + 
  annotate(geom = "text", x = ggpp::as_npc(0.684), y = ggpp::as_npc(0.97), label = "(c)", size = (10*0.35), family = "serif"); p1
...
p7 <- ggplot() + 
  ...
  theme(axis.line = element_blank(), 
        ...
        plot.margin = margin(t = 0, r = 1, b = 0, l = 0), 
        panel.grid = element_blank()) + 
  annotate(geom = "text", x = 0.5, y = 2.5, label = "Total damage ratio [%]", size = (10*0.35), family = "serif", angle = 90); p7

p1 <- cowplot::plot_grid(p7, p1, nrow = 1, rel_widths = c(0.4, 15)); p1
...
p7 <- ggplot() + 
  ...
  geom_rect(aes(xmin = 0.2, xmax = 1.8, ymin = 0.81, ymax = 0.71), fill = "#d62728", alpha = 0.2) + 
  geom_rect(aes(xmin = 0.6, xmax = 1.4, ymin = 0.86, ymax = 0.66), fill = "#d62728", alpha = 0.6) + 
  geom_point(aes(x = 1, y = 0.76), color = "#d62728", fill = "#ffffff", shape = 19, size = 2.5) + 
  annotate(geom = "text", x = 1, y = 0.3, label = "Uprooting", size = (9*0.35), family = "serif") + 

  geom_rect(aes(xmin = 2.2, xmax = 3.8, ymin = 0.81, ymax = 0.71), fill = "#ff7f0e", alpha = 0.2) + 
  geom_rect(aes(xmin = 2.6, xmax = 3.4, ymin = 0.86, ymax = 0.66), fill = "#ff7f0e", alpha = 0.6) + 
  geom_point(aes(x = 3, y = 0.76), color = "#ff7f0e", fill = "#ffffff", shape = 19, size = 2.5) + 
  annotate(geom = "text", x = 3, y = 0.3, label = "Clear-bole broken", size = (9*0.35), family = "serif") + 

  geom_rect(aes(xmin = 4.2, xmax = 5.8, ymin = 0.81, ymax = 0.71), fill = "#1f77b4", alpha = 0.2) + 
  geom_rect(aes(xmin = 4.6, xmax = 5.4, ymin = 0.86, ymax = 0.66), fill = "#1f77b4", alpha = 0.6) + 
  geom_point(aes(x = 5, y = 0.76), color = "#1f77b4", fill = "#ffffff", shape = 19, size = 2.5) + 
  annotate(geom = "text", x = 5, y = 0.3, label = "Crown broken", size = (9*0.35), family = "serif") + 
  
  geom_rect(aes(xmin = 6.2, xmax = 7.8, ymin = 0.81, ymax = 0.71), fill = "#2ca02c", alpha = 0.2) + 
  geom_rect(aes(xmin = 6.6, xmax = 7.4, ymin = 0.86, ymax = 0.66), fill = "#2ca02c", alpha = 0.6) + 
  geom_point(aes(x = 7, y = 0.76), color = "#2ca02c", fill = "#ffffff", shape = 19, size = 2.5) + 
  annotate(geom = "text", x = 7, y = 0.3, label = "Branch broken", size = (9*0.35), family = "serif") + 

  geom_rect(aes(xmin = 9.5, xmax = 11, ymin = 0.70, ymax = 0.80), fill = "#d62728", alpha = 0.2) + 
  annotate(geom = "text", x = 11.15, y = 0.75, label = "95% CI", size = (9*0.35), family = "serif", hjust = "left") + 
  geom_rect(aes(xmin = 9.9, xmax = 10.6, ymin = 0.10, ymax = 0.40), fill = "#d62728", alpha = 0.6) + 
  annotate(geom = "text", x = 11.15, y = 0.25, label = "± SE", size = (9*0.35), family = "serif", hjust = "left") + 

  geom_point(aes(x = 8.15, 0.75), color = "#d62728", fill = "#ffffff", shape = 21, size = 2.5) + 
  annotate(geom = "text", x = 8.3, y = 0.75, label = expression(italic(P)*" ≥ 0.05"), size = (9*0.35), family = "serif", hjust = "left") + 
  geom_point(aes(x = 8.15, 0.25), color = "#d62728", fill = "#ffffff", shape = 19, size = 2.5) + 
  annotate(geom = "text", x = 8.3, y = 0.25, label = expression(italic(P)*" < 0.05"), size = (9*0.35), family = "serif", hjust = "left"); p7

p2 <- cowplot::plot_grid(cowplot::plot_grid(p4, p5, p6, nrow = 1), p7, nrow = 2, rel_heights = c(5, 0.8)) + 
  annotate(geom = "text", x = ggpp::as_npc(0.014), y = ggpp::as_npc(0.97), label = "(d)", size = (10*0.35), family = "serif") + 
  annotate(geom = "text", x = ggpp::as_npc(0.349), y = ggpp::as_npc(0.97), label = "(e)", size = (10*0.35), family = "serif") + 
  annotate(geom = "text", x = ggpp::as_npc(0.684), y = ggpp::as_npc(0.97), label = "(f)", size = (10*0.35), family = "serif"); p2
...
p7 <- ggplot() + 
  ...
  theme(axis.line = element_blank(), 
        ...
        plot.margin = margin(t = 0, r = 1, b = 0, l = 0), 
        panel.grid = element_blank()) + 
  annotate(geom = "text", x = 0.5, y = 2.5, label = "Category-specific damage", size = (10*0.35), family = "serif", angle = 90); p7

cowplot::plot_grid(p1, cowplot::plot_grid(p7, p2, rel_widths = c(0.4, 15)), nrow = 2, align = "hv", rel_heights = c(5.4, 4.6)) + 
  geom_segment(aes(x = ggpp::as_npc(0.3506), y = ggpp::as_npc(0.15), xend = ggpp::as_npc(0.3506), yend = ggpp::as_npc(0.95)), color = "#808080", linetype = 5, alpha = 0.5, linewidth = 0.4) + 
  geom_segment(aes(x = ggpp::as_npc(0.6753), y = ggpp::as_npc(0.15), xend = ggpp::as_npc(0.6753), yend = ggpp::as_npc(0.95)), color = "#808080", linetype = 5, alpha = 0.5, linewidth = 0.4)
# ggsave(file = "3_save/fig_2_20260620.tiff", width = 16, height = 10, units = "cm", dpi = 300, limitsize = FALSE, bg = "#ffffff")
rm(data, data_anal, model, p1, p2, p3, p4, p5, p6, p7, p8, p9)
...
# ____3.3.2 Fig 3B ---------------------------------------------------------------------------------
p2 <- data_plot$var$contrib |> as_tibble(rownames = "names") |> 
  ...
  theme(# axis.ticks.y = element_blank(), 
    ...
    axis.line.x = element_line(color = "#000000", size = 0.5)); p2

# ____3.3.3 Fig 3C ---------------------------------------------------------------------------------
p3 <- data_plot$var$contrib |> as_tibble(rownames = "names") |> 
  mutate(names = factor(names, levels = c("P50", "woody_density", "rdmax", "height", "dispersal_agent", "seed_width", "seed_length", "seed_mass", "LeafN", "SLA"), 
    labels = c("P50", "WD", "RDmax", "Hmax", "DM", "SW", "SL", "SM", "LeafN", "SLA"))) |> 
  ...
  theme(# axis.ticks.y = element_blank(), 
    panel.border = element_blank(), 
    plot.title = element_text(hjust = 0.5, face = "plain"), 
    axis.line.x = element_line(color = "#000000", size = 0.5)); p3

p2 <- cowplot::plot_grid(p2, p3, nrow = 2); p2
p1 <- cowplot::plot_grid(p1, p2, nrow = 1, rel_widths = c(2, 1)); p1
...
p3 <- cowplot::plot_grid(p3, p4, nrow = 1); p3
cowplot::plot_grid(p1, p3, nrow = 2, rel_heights = c(8, 4)) + 
...
# ggsave(file = "save/fig_3_20260620.tiff", width = 16, height = 16, units = "cm", dpi = 300, limitsize = FALSE, bg = "#ffffff")
rm(data, data_anal, data_plot, model, p1, p2, p3, p4)

# __3.4 figure 4 -----------------------------------------------------------------------------------
load(file = "save/model_set_20260620.rdata")
...
# ____3.4.1 Fig 4A ---------------------------------------------------------------------------------
p1 <- ggplot(data, aes(x = 1, y = ratio)) + 
...
# ____3.4.2 Fig 4B ---------------------------------------------------------------------------------
p2 <- ggplot(data, aes(x = Estimate, y = desc(Var))) + 
...
cowplot::plot_grid(p1, p2, nrow = 1, align = "h", axis = "b", rel_widths = c(2.5, 7.5)) + 
...
# ggsave(file = "save/fig_4_20260620.tiff", width = 14, height = 16, units = "cm", dpi = 300, limitsize = FALSE, bg = "#ffffff")
rm(model_avg, model_set, p1, p2, data)
...
# ggsave(file = "save/fig_5_20260620.tiff", width = 13.2, height = 4.8, units = "cm", dpi = 300, limitsize = FALSE, bg = "#ffffff")
...
data_anal <- data_indi |> mutate(damage_1 = ifelse(n_damage_1 == n_stem, 1, 0), damage_2 = ifelse(n_damage_2 == n_stem, 1, 0), damage_3 = ifelse(n_damage_3 == n_stem, 1, 0), 
  damage_4 = ifelse(n_damage_4 == n_stem, 1, 0), damage_5 = ifelse(n_damage_5 == n_stem, 1, 0)) |> separate(quadrat, into = c("site", "plot"), remove = FALSE) |> 
  mutate(bin_stem = ifelse(n_stem > 1, "M", "S"), .after = n_stem) |> mutate(bin_stem = factor(bin_stem, levels = c("S", "M"), labels = c("single-stemmed", "multi-stemmed")))
model <- glmmTMB(cbind(n_damage_2, n_stem - n_damage_2) ~ n_stem + (1 | site) + (1 | species_LN), family = binomial, data = data_anal)
summary(model); data_anal |> summarise(min = min(n_stem), max = max(n_stem))
data <- effects::allEffects(model, xlevels = list(n_stem = seq(0.4, 18, 0.01)))[[1]] |> as.data.frame()

p1 <- ggplot() + 
  geom_jitter(data = data_anal, aes(x = n_stem, y = DR_damage_2), color = "#bfbfbf", shape = 16, size = 1.6, alpha = 0.7) + 
  geom_ribbon(data = data, aes(x = n_stem, y = fit, ymin = lower, ymax = upper), fill = "#aa71a4", alpha = 0.2) + 
  geom_line(data = data, aes(x = n_stem, y = fit), color = "#aa71a4", linetype = 2) + 
  scale_x_continuous(limits = c(0, 18), breaks = c(0, 9, 18)) + 
  scale_y_continuous(limits = c(0, 1), breaks = c(0, 0.5, 1.0)) + 
  labs(x = "Stem abundance", y = "Total damage ratio [%]") + 
  geom_text(aes(x = ggpp::as_npc(0.50), y = ggpp::as_npc(0.95)), label = "Uprooting", size = (9*0.35), family = "serif", hjust = "left") + 
  geom_text(aes(x = ggpp::as_npc(0.50), y = ggpp::as_npc(0.85)), label = expression(italic(P)*" = 0.284"), size = (9*0.35), family = "serif", hjust = "left") + 
  Sylvan + 
...
data_anal <- data_indi |> mutate(damage_1 = ifelse(n_damage_1 == n_stem, 1, 0), damage_2 = ifelse(n_damage_2 == n_stem, 1, 0), damage_3 = ifelse(n_damage_3 == n_stem, 1, 0), 
  damage_4 = ifelse(n_damage_4 == n_stem, 1, 0), damage_5 = ifelse(n_damage_5 == n_stem, 1, 0)) |> separate(quadrat, into = c("site", "plot"), remove = FALSE) |> 
  mutate(bin_stem = ifelse(n_stem > 1, "M", "S"), .after = n_stem) |> mutate(bin_stem = factor(bin_stem, levels = c("S", "M"), labels = c("single-stemmed", "multi-stemmed")))
model <- glmmTMB(cbind(n_damage_3, n_stem - n_damage_3) ~ n_stem + (1 | site) + (1 | species_LN), family = binomial, data = data_anal)
summary(model); data_anal |> summarise(min = min(n_stem), max = max(n_stem))
data <- effects::allEffects(model, xlevels = list(n_stem = seq(0.4, 18, 0.01)))[[1]] |> as.data.frame()

p2 <- ggplot() + 
  geom_jitter(data = data_anal, aes(x = n_stem, y = DR_damage_3), color = "#bfbfbf", shape = 16, size = 1.6, alpha = 0.7) + 
  geom_ribbon(data = data, aes(x = n_stem, y = fit, ymin = lower, ymax = upper), fill = "#aa71a4", alpha = 0.2) + 
  geom_line(data = data, aes(x = n_stem, y = fit), color = "#aa71a4", linetype = 2) + 
  scale_x_continuous(limits = c(0, 18), breaks = c(0, 9, 18)) + 
  scale_y_continuous(limits = c(0, 1), breaks = c(0, 0.5, 1.0)) + 
  labs(x = "Stem abundance", y = "Total damage ratio [%]") + 
  geom_text(aes(x = ggpp::as_npc(0.50), y = ggpp::as_npc(0.95)), label = "Clear-bole broken", size = (9*0.35), family = "serif", hjust = "left") + 
  geom_text(aes(x = ggpp::as_npc(0.50), y = ggpp::as_npc(0.85)), label = expression(italic(P)*" = 0.626"), size = (9*0.35), family = "serif", hjust = "left") + 
  Sylvan + 
  theme(legend.position = "none"); p2
...
data_anal <- data_indi |> mutate(damage_1 = ifelse(n_damage_1 == n_stem, 1, 0), damage_2 = ifelse(n_damage_2 == n_stem, 1, 0), damage_3 = ifelse(n_damage_3 == n_stem, 1, 0), 
  damage_4 = ifelse(n_damage_4 == n_stem, 1, 0), damage_5 = ifelse(n_damage_5 == n_stem, 1, 0)) |> separate(quadrat, into = c("site", "plot"), remove = FALSE) |> 
  mutate(bin_stem = ifelse(n_stem > 1, "M", "S"), .after = n_stem) |> mutate(bin_stem = factor(bin_stem, levels = c("S", "M"), labels = c("single-stemmed", "multi-stemmed")))
model <- glmmTMB(cbind(n_damage_4, n_stem - n_damage_4) ~ n_stem + (1 | site) + (1 | species_LN), family = binomial, data = data_anal)
summary(model); data_anal |> summarise(min = min(n_stem), max = max(n_stem))
data <- effects::allEffects(model, xlevels = list(n_stem = seq(0.4, 18, 0.01)))[[1]] |> as.data.frame()

p3 <- ggplot() + 
  geom_jitter(data = data_anal, aes(x = n_stem, y = DR_damage_4), color = "#bfbfbf", shape = 16, size = 1.6, alpha = 0.7) + 
  geom_ribbon(data = data, aes(x = n_stem, y = fit, ymin = lower, ymax = upper), fill = "#aa71a4", alpha = 0.2) + 
  geom_line(data = data, aes(x = n_stem, y = fit), color = "#aa71a4") + 
  scale_x_continuous(limits = c(0, 18), breaks = c(0, 9, 18)) + 
  scale_y_continuous(limits = c(0, 1), breaks = c(0, 0.5, 1.0)) + 
  labs(x = "Stem abundance", y = "Total damage ratio [%]") + 
  geom_text(aes(x = ggpp::as_npc(0.50), y = ggpp::as_npc(0.95)), label = "Crown broken", size = (9*0.35), family = "serif", hjust = "left") + 
  geom_text(aes(x = ggpp::as_npc(0.50), y = ggpp::as_npc(0.85)), label = expression(italic(P)*" < 0.001***"), size = (9*0.35), family = "serif", hjust = "left") + 
  Sylvan + 
  theme(legend.position = "none"); p3
...
data_anal <- data_indi |> mutate(damage_1 = ifelse(n_damage_1 == n_stem, 1, 0), damage_2 = ifelse(n_damage_2 == n_stem, 1, 0), damage_3 = ifelse(n_damage_3 == n_stem, 1, 0), 
  damage_4 = ifelse(n_damage_4 == n_stem, 1, 0), damage_5 = ifelse(n_damage_5 == n_stem, 1, 0)) |> separate(quadrat, into = c("site", "plot"), remove = FALSE) |> 
  mutate(bin_stem = ifelse(n_stem > 1, "M", "S"), .after = n_stem) |> mutate(bin_stem = factor(bin_stem, levels = c("S", "M"), labels = c("single-stemmed", "multi-stemmed")))
model <- glmmTMB(cbind(n_damage_5, n_stem - n_damage_5) ~ n_stem + (1 | site) + (1 | species_LN), family = binomial, data = data_anal)
summary(model); data_anal |> summarise(min = min(n_stem), max = max(n_stem))
data <- effects::allEffects(model, xlevels = list(n_stem = seq(0.4, 18, 0.01)))[[1]] |> as.data.frame()

p4 <- ggplot() + 
  geom_jitter(data = data_anal, aes(x = n_stem, y = DR_damage_5), color = "#bfbfbf", shape = 16, size = 1.6, alpha = 0.7) + 
  geom_ribbon(data = data, aes(x = n_stem, y = fit, ymin = lower, ymax = upper), fill = "#aa71a4", alpha = 0.2) + 
  geom_line(data = data, aes(x = n_stem, y = fit), color = "#aa71a4") + 
  scale_x_continuous(limits = c(0, 18), breaks = c(0, 9, 18)) + 
  scale_y_continuous(limits = c(0, 1), breaks = c(0, 0.5, 1.0)) + 
  labs(x = "Stem abundance", y = "Total damage ratio [%]") + 
  geom_text(aes(x = ggpp::as_npc(0.50), y = ggpp::as_npc(0.95)), label = "Branch broken", size = (9*0.35), family = "serif", hjust = "left") + 
  geom_text(aes(x = ggpp::as_npc(0.50), y = ggpp::as_npc(0.85)), label = expression(italic(P)*" < 0.001***"), size = (9*0.35), family = "serif", hjust = "left") + 
  Sylvan + 
  theme(legend.position = "none"); p4

cowplot::plot_grid(p1, p2, p3, p4, nrow = 2) + 
  annotate(geom = "text", x = ggpp::as_npc(0.018), y = ggpp::as_npc(0.98), label = "(a)", size = (10*0.35), family = "serif") + 
  annotate(geom = "text", x = ggpp::as_npc(0.518), y = ggpp::as_npc(0.98), label = "(b)", size = (10*0.35), family = "serif") + 
  annotate(geom = "text", x = ggpp::as_npc(0.018), y = ggpp::as_npc(0.48), label = "(c)", size = (10*0.35), family = "serif") + 
  annotate(geom = "text", x = ggpp::as_npc(0.518), y = ggpp::as_npc(0.48), label = "(d)", size = (10*0.35), family = "serif")
# ggsave(file = "3_save/fig_S2_20260620.tiff", width = 13, height = 12, units = "cm", dpi = 300, limitsize = FALSE, bg = "#ffffff")
rm(data_anal, model, p1, p2, p3, p4)
...
p1 <- ggplot() + 
  ...
  labs(x = "Stem abundance", y = "Total damage ratio [%]") + 
  geom_text(aes(x = ggpp::as_npc(0.05), y = ggpp::as_npc(0.95)), label = "Uprooting", size = (9*0.35), family = "serif", hjust = "left") + 
...
p2 <- ggplot() + 
  ...
  labs(x = "Stem abundance", y = "Total damage ratio [%]") + 
  geom_text(aes(x = ggpp::as_npc(0.05), y = ggpp::as_npc(0.95)), label = "Clear-bole broken", size = (9*0.35), family = "serif", hjust = "left") + 
  geom_text(aes(x = ggpp::as_npc(0.75), y = ggpp::as_npc(0.95)), label = expression(italic(P)*" < 0.001***"), size = (9*0.35), family = "serif") + 
  Sylvan + 
  theme(legend.position = "none"); p2
...
p3 <- ggplot() + 
  ...
  labs(x = "Stem abundance", y = "Total damage ratio [%]") + 
  geom_text(aes(x = ggpp::as_npc(0.05), y = ggpp::as_npc(0.95)), label = "Crown broken", size = (9*0.35), family = "serif", hjust = "left") + 
...
p4 <- ggplot() + 
  ...
  labs(x = "Stem abundance", y = "Total damage ratio [%]") + 
  geom_text(aes(x = ggpp::as_npc(0.05), y = ggpp::as_npc(0.95)), label = "Branch broken", size = (9*0.35), family = "serif", hjust = "left") + 
...
cowplot::plot_grid(p1, p2, p3, p4, nrow = 2) + 
  ...
  annotate(geom = "text", x = ggpp::as_npc(0.018), y = ggpp::as_npc(0.48), label = "(c)", size = (10*0.35), family = "serif") + 
  annotate(geom = "text", x = ggpp::as_npc(0.518), y = ggpp::as_npc(0.48), label = "(d)", size = (10*0.35), family = "serif")
# ggsave(file = "3_save/fig_S3_20260620.tiff", width = 13, height = 12, units = "cm", dpi = 300, limitsize = FALSE, bg = "#ffffff")
rm(data, data_anal, model, p1, p2, p3, p4)
...
cowplot::plot_grid(p1, p2, p3, p4, nrow = 2) + 
  ...
  annotate(geom = "text", x = ggpp::as_npc(0.018), y = ggpp::as_npc(0.48), label = "(c)", size = (10*0.35), family = "serif") + 
  annotate(geom = "text", x = ggpp::as_npc(0.518), y = ggpp::as_npc(0.48), label = "(d)", size = (10*0.35), family = "serif")
# ggsave(file = "3_save/fig_S4_20260620.tiff", width = 13, height = 12, units = "cm", dpi = 300, limitsize = FALSE, bg = "#ffffff")
rm(data, data_anal, model, p1, p2, p3, p4)
